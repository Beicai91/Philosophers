...
static int  get_start_index(char *str)
{
    ...
    while (is_space(str[i]))
        i++;
    if (str[i] == '-')
        error_exit("Invalid arguments: you probably entered a negative number", NULL);
    else if (str[i] == '+')
        i++;
    if (is_digit(str[i]))
    {
...
//example of a valid str "       +376&*%" max 2147483647
...
void    check_args_and_fill(t_simulation *simulation, char **argv)
{
    ...
    simulation->time_to_die = ft_atol(argv[2]) * 1000; //convert to microsecond for usleep
