...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/06/20 11:39:38 by bcai              #+#    #+#             */
/*   Updated: 2024/06/20 12:10:40 by bcai             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	mutex_error_handling(int value_returned, t_opconst operation,
		t_simulation *simulation)
{
	if ((value_returned == EINVAL || value_returned == ENOMEM)
		&& operation == INIT)
		error_exit("Mutex initiation failed.", simulation);
	else if ((value_returned == EINVAL || value_returned == EDEADLK)
		&& operation == LOCK)
		error_exit("Mutex lock failed", simulation);
	else if (value_returned == ENOMEM)
		error_exit("The process cannot allocate enough memory \
			to create another mutex.", simulation);
	else if ((value_returned == EDEADLK || value_returned == EPERM)
		&& operation == UNLOCK)
		error_exit("Mutex unlock failed.", simulation);
	else if ((value_returned == EBUSY || value_returned == EINVAL)
		&& operation == DESTROY)
		...
		t_simulation *simulation)
{
	if (operation == INIT)
		...
		error_exit("Wrong operation on mutex.", simulation);
}
